...
def parse_verilog_indexes(verilog_path):
	...
	sections = content.split('/*Â§Â§#Â§Â§*/')
	if len(sections) < 3:
		raise ValueError("Could not find two '§§#§§' markers in the file.")
	section = sections[1]
	input_dict = {}
	output_dict = {}
	...
	for line in section.splitlines():
		...
		if len(parts) > 0 and parts[0]:
			m = re.match(r'localparam\s+([A-Z0-9_]+)\s*=\s*(\d+)', parts[0])
			if m:
				name, idx = m.groups()
				input_dict[str.lower(name)] = int(idx)
		# Process output column
		if len(parts) > 1 and parts[1]:
			# There may be multiple localparams in the output column
			for m in re.finditer(r'localparam\s+([A-Z0-9_]+)\s*=\s*(\d+)', parts[1]):
				name, idx = m.groups()
...
outputOrders = [
	...
	"dig0", "dig1", 
	"asg_ampl0", "asg_ampl1", ]
def orderDictionary(dict, keysInOrder, defaultValue):
	'''keys in dict not present in keysInOrder will be left at the end of the list
	keys in keysInOrder not present in dict will be skipped'''
	orderedDict = OrderedDict()
	for key in keysInOrder:
		if key in dict:
			orderedDict[key] = dict[key]
		else:
			orderedDict[key] = defaultValue
	orderedDict.update(dict)
	return orderedDict
