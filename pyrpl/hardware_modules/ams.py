...
class AmsNouveau(HardwareModule):
    ...
    ramp_useMultiTriggers = BoolRegister(0x60,0x13,
                                 doc="if false, one trigger will start the entire ramp function. If true, a trigger will only start one of the segments of the ramp, and thus you'll need 'ramp_numberOfUsedRamps' triggers to finish the entire ramp function. When using multiple triggers, if the current segment is finished but no new trigger has been received yet, the PWM will stay still at the last value reached until a new trigger is received. Instead, if a trigger is received before the current segment is finished, the next segment of the ramp function will start immediately")
