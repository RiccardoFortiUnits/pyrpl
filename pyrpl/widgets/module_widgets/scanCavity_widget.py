...
class PeakLine(QtWidgets.QGraphicsLineItem):
	...
	def updateBarPositions(self):
		...
		self.targetLine.setLine((self.line().x1() + self.line().x2()) / 2, self.line().y1(), (self.line().x1() + self.line().x2()) / 2, self.line().y2() - self.barHeight/2)
	...
	def targetLineWidths(self):
		return self.barWidths * 0.1
	@property
	def barHeight(self):
		return self.strokeWidth * 25		
	# @property
...
class scanSwitcher:
	...
	def _on_mouse_moved(self, scene_pos):
		# only update while a point selection is active
		if not getattr(self, "_point_selection_active", False):
			return
		vb = self.scanCavity.plot_item.getViewBox()
		if vb is None:
			return
		rect = vb.sceneBoundingRect()
		x = scene_pos.x()
		# draw a vertical line spanning the viewBox in scene coordinates
		self.rangeLine.setLine(x, rect.top(), x, rect.bottom())

	def _click_finalizer(self, event):
		# call original handler, then stop tracking and hide the line
		try:
			self.get_point(event)
		finally:
			...
			try:
				...
				pass
			try:
				scene.sigMouseClicked.disconnect(self._click_finalizer)
			except Exception:
				pass
			self._point_selection_active = False
			if hasattr(self, "rangeLine"):
				self.rangeLine.setVisible(False)

	def enable_point_selection(self):
		# initialize viewBox and the rangeLine if necessary, then start tracking
		self.viewBox = self.scanCavity.plot_item.getViewBox()
		scene = self.viewBox.scene()
		if not hasattr(self, "rangeLine") or self.rangeLine is None:
			self.rangeLine = QtWidgets.QGraphicsLineItem(0.0, 0, 1, 0)			
			self.scanCavity.plot_item.addItem(self.rangeLine)
			pen = QtGui.QPen(QtGui.QColor(0, 255, 0, 0), .1)
			self.rangeLine.setPen(pen)
			self.scanCavity.plot_item.addItem(self.rangeLine)
			self.rangeLine.setZValue(100)  # keep it on top
		self.rangeLine.setVisible(True)
		self._point_selection_active = True
		# connect handlers (avoid duplicate connections)
		try:
			scene.sigMouseMoved.disconnect(self._on_mouse_moved)
		except Exception:
			pass
		...
		scene.sigMouseClicked.connect(self._click_finalizer)

	def get_point(self, event):
		if event.button() == QtCore.Qt.LeftButton:
			pos = self.viewBox.mapSceneToView(event.scenePos())
			self.call_function_with_position(pos)

	def call_function_with_position(self, position):
		# Replace this with the actual function you want to call
		print(f"Selected position: {position.x()}, {position.y()}")
		

	def __init__(self, scanCavity : ScanCavity_widget):
		self.button = QtWidgets.QPushButton("Select Point")
