...
class ColorDialogWithWavelength(QtWidgets.QColorDialog):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.setOption(QtWidgets.QColorDialog.ShowAlphaChannel, False)
        # Add wavelength input below the color dialog
        self.wavelength_label = QtWidgets.QLabel("Wavelength (nm, 380-780):")
        self.wavelength_input = QtWidgets.QSpinBox()
        self.wavelength_input.setRange(380, 780)
        self.wavelength_input.setValue(556)
        self.wavelength_input.valueChanged.connect(self.set_color_from_wavelength)
        # Layout for wavelength controls
        wl_layout = QtWidgets.QHBoxLayout()
        wl_layout.addWidget(self.wavelength_label)
        wl_layout.addWidget(self.wavelength_input)
        # Insert into dialog layout
        main_layout = self.layout()
        main_layout.addLayout(wl_layout)

    def set_color_from_wavelength(self):
    ...
    def wavelength_to_rgb(wavelength):
        # Approximate conversion for visible spectrum
...
class ColorAttributeWidget(BaseAttributeWidget):
    ...
    def _make_widget(self):
        ...
        # Open the standard color dialog with an extra wavelength input

    def open_color_dialog(self):
        initial = self.widget.palette().button().color()
        dialog = ColorDialogWithWavelength(initial, self)
